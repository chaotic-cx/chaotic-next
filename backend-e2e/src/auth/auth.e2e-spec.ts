...
describe('Auth & Protected Endpoints (e2e)', () => {
  let e2e: E2eApp;
  let realAuthApp: NestFastifyApplication;

  beforeAll(async () => {
    e2e = await createE2eApp();

    const moduleRef = await Test.createTestingModule({ imports: [AppModule] }).compile();
    realAuthApp = moduleRef.createNestApplication<NestFastifyApplication>(new FastifyAdapter(), {
      logger: false,
    });
    await realAuthApp.init();
  });
  ...
  describe('Public endpoints (unprotected)', () => {
    it('GET /api/auth/ok is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/api/auth/ok' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /health is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/health' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /health/ready is accessible without authentication (not 401)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/health/ready' });
      expect(res.statusCode).not.toBe(401);
    });

    it('GET /gitlab/pipelines is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/gitlab/pipelines' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /gitlab/review-stats is accessible without authentication (not 401)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/gitlab/review-stats' });
      expect(res.statusCode).not.toBe(401);
    });
    ...
    it('GET /builder/builders is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/builder/builders' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /builder/packages is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/builder/packages' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /builder/repos is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/builder/repos' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /builder/builds is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/builder/builds' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /builder/latest is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/builder/latest' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /metrics/users is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/metrics/users' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /metrics/user-agents is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/metrics/user-agents' });
      expect(res.statusCode).toBe(200);
    });

    it('GET /router/country/7 is accessible without authentication (200)', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/router/country/7' });
      expect(res.statusCode).toBe(200);
    });
  });
  ...
  describe('Protected endpoints (AuthGuard)', () => {
    it('GET /admin/packages returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/admin/packages' });
      expect(res.statusCode).toBe(401);
    });

    it('PATCH /admin/packages/1 returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'PATCH', url: '/admin/packages/1', payload: {} });
      expect(res.statusCode).toBe(401);
    });

    it('DELETE /admin/packages/1 returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'DELETE', url: '/admin/packages/1' });
      expect(res.statusCode).toBe(401);
    });

    it('POST /admin/repos returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'POST', url: '/admin/repos', payload: {} });
      expect(res.statusCode).toBe(401);
    });

    it('PATCH /admin/repos/1 returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'PATCH', url: '/admin/repos/1', payload: {} });
      expect(res.statusCode).toBe(401);
    });

    it('DELETE /admin/repos/1 returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'DELETE', url: '/admin/repos/1' });
      expect(res.statusCode).toBe(401);
    });

    it('DELETE /admin/arch-packages/1 returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'DELETE', url: '/admin/arch-packages/1' });
      expect(res.statusCode).toBe(401);
    });
    ...
    it('GET /repo/run returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/repo/run' });
      expect(res.statusCode).toBe(401);
    });

    it('GET /repo/signal-scan returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/repo/signal-scan' });
      expect(res.statusCode).toBe(401);
    });

    it('GET /repo/broken returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/repo/broken' });
      expect(res.statusCode).toBe(401);
    });

    it('POST /repo/index/arch returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'POST', url: '/repo/index/arch' });
      expect(res.statusCode).toBe(401);
    });
    ...
    it('GET /repo/dependencies returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/repo/dependencies' });
      expect(res.statusCode).toBe(401);
    });

    it('GET /repo/dependencies/firedragon returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({ method: 'GET', url: '/repo/dependencies/firedragon' });
      expect(res.statusCode).toBe(401);
    });
    ...
    it('POST /repo/signals/import-file returns 401 when unauthenticated', async () => {
      const res = await realAuthApp.inject({
        method: 'POST',
        url: '/repo/signals/import-file',
